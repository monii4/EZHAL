...
Simulates operating states and line contingencies on a standard 9-bus test system.

Uses physics-constrained optimization to generate target actions.

Trains supervised Extra Trees models:

Classifier: selects the required action type.

Regressor: predicts where and how much support/headroom is required.

Applies ML decisions back to a pandapower AC power-flow model.

Uses a physics-based safety layer when a direct ML action does not satisfy the security criterion.

Presents live ML inference and live physics validation through a Streamlit dashboard.
